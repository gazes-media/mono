...
export class AnimeHighlightedRoute extends Route {
	...
	public handler: RouteHandlerMethod = async (request, reply) => {
		/* The function `pickRandomAnimeWithAtLeastAsynopsisDecent` picks a 
		random anime from a store and checks if it has a synopsis with at least 
		32 words and a non-default cover image, and if not, recursively calls 
		itself until a suitable anime is found. */
			...
			return new Promise((resolve, reject) => {
				AnimeStore.get(randomAnime.id.toString(), "vostfr").then((anime) => {
					...
					if (
						wordLength > 32 &&
						anime.coverUrl !=
						"https://neko-sama.fr/images/default_background.png"
					) {
						resolve(anime);
					} else {
